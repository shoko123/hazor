...
import type { TApiTrio2, TApiGroupColumn, TGroupUnion, TGroupLocalUnion, TApiGroupTag, TApiGroupBasic, TParamObj, TTrio, TGroupObj, TCategoriesArray, TGroupLocalBase, TGroupLocalColumn, TGroupLocalTag, TParam, TParamLocal, TApiParamNameAndId, TApiParamName } from '../../../types/trioTypes2'
import { TGroup } from '@/js/types/trioTypes'
...
export const useTrioNormalizerStore2 = defineStore('trioNorm2', () => {
  ...
  function normalizeTrio2(apiTrio: TApiTrio2) {
    ...
    function saveGroupAndParams(grpKey: string, grp: TGroupUnion) {
      ...
      //remove params array
      let grpToSave1 = (({ params, ...object }) => object)(grpWithParams)

      //give type
      let grpToSave: TGroupLocalUnion = { ...(({ params, ...object }) => object)(grpWithParams) }

      console.log(`toSave: ${JSON.stringify(grpToSave, null, 2)}`);
      ...
      console.log(`group ${prmCnt} done`);
    }


    return { categories, groupsObj, paramsObj }
  }
