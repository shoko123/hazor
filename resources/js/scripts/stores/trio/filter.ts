...
export const useFilterStore = defineStore('filter', () => {
  ...
  const { trio, groupLabelToKey, orderByAvailable, orderByGroup, orderByOptions, currentGroup } = storeToRefs(useTrioStore())
  const selectedFilterParams = ref<string[]>([])

  function filtersToQueryObject() {
    const q2: IStringObject = {}
    selectedFilterParams.value.sort((a, b) => { return a > b ? 1 : -1 })
    selectedFilterParams.value.forEach(k => {
      const paramUlined = trio.value.paramsObj[k].text.replace(/ /g, "_")
      const groupUlined = trio.value.groupsObj[trio.value.paramsObj[k].groupKey].label.replace(/ /g, "_")
  ...
  const apiQueryPayload = computed<TApiFilters>(() => {
    ...
    if (trio.value.categories.length === 0) { return all }
  ...
  function orderParamClicked(index: number, asc: boolean) {
    const orderByParams = orderByGroup.value?.paramKeys.map(x => { return { ...trio.value.paramsObj[x], key: x } })
