...
export const useCollectionMainStore = defineStore('collectionMain', () => {
  ...
  const loadPage: TFuncLoadPage = async function (pageNoB1: number, view: TCView, module: TModule) {
    ...
    console.log(
      `collectionMain.loadPage() view: ${view.name} pageB1: ${pageNoB1}  ipp: ${ipp} startIndex: ${start} endIndex: ${start + ipp - 1} module: ${module} `,
    )
    ...
    const res = await send<TApiPage[]>('model/page', 'post', {
      model: module,
      view: view.name,
      ids,
    })
  ...
  function savePage(apiPage: TApiPage[], view: TCView, module: TModule): void {
    ...
    switch (view.name) {
      ...
      case 'Tabular':
        toSave = (<TApiPageMainTabular[]>apiPage).map((x) => {
          return { ...x, tag: tagFromSlug(module, x.slug) }
        })
        page.value = toSave
        break

      case 'Chips':
        toSave = (<TApiArrayMain[]>apiPage).map((x) => {
          return { ...x, tag: tagFromSlug(module, x.slug) }
        })
